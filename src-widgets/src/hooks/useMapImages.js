...
const useMapImages = (styleSelect) => {
					...
					loadedImages.push(
						new Promise((resolve, reject) => {
							map.getMap().loadImage(image.png, (error, mapImage) => {
								...
								map.getMap().addImage(image.id, mapImage, {
									sdf: image.id !== "icon-traffic-closure",
								});
								resolve();
							});
						}).catch((ex) =>
							console.log(`useMapImages() -> loadImages() -> Error: ${ex}`),
						),
					);
				}
			}
	...
	useEffect(() => {
		...
		mapRef.current
			.getMap()
			.setStyle(`mapbox://styles/mapbox/${styleSelect || "streets-v12"}`, {
				diff: false,
			});
		loadImages();
	}, [styleSelect, loadImages]);
